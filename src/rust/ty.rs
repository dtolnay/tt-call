...
macro_rules! private_parse_type {
    ...
    // or array.
    {
        ...
        tt_call! {
            macro = [{ private_validate_inside_brackets }]
            original = [{ $original }]
            input = [{ $($bracketed)* }]
            ~~> tt_return! {
                $caller
    ...
    // Unexpected: first token is empty brackets
    {
        ...
        private_unexpected_close_empty_square_brackets! {
            $first
        }
    };
    ...
    // First token is a parenthesized group. Validate and return as tuple type.
    {
        ...
        tt_call! {
            macro = [{ private_validate_inside_parens }]
            input = [{ $($parenthesized)* }]
            ~~> tt_return! {
                $caller
                type = [{ $original }]
    ...
    // Angle brackets are fully qualified trait syntax with absolute path.
    {
        ...
        type_prefix = [{ $lt:tt }]
        type = [{ $($qself:tt)* }]
        rest = [{ as :: $segment:ident $($rest:tt)* }]
        _rest = [{ $as:tt $colons:tt $($dup:tt)* }]
    } => {
        tt_call! {
    ...
    // Close angle bracket of fully qualified trait syntax. Parse rest of path.
    {
        ...
        qpath = [{ $($qpath:tt)* }]
        path = [{ $($path:tt)* }]
        rest = [{ > :: $segment:ident $($rest:tt)* }]
        _rest = [{ $gt:tt $colons:tt $($dup:tt)* }]
    } => {
        tt_call! {
            macro = [{ private_parse_possibly_empty_path_after_ident }]
            input = [{ $($rest)* }]
...
macro_rules! private_parse_function {
    ...
    // Validate parenthesized function arguments.
    {
        ...
        tt_call! {
            macro = [{ private_validate_fn_args }]
            input = [{ $($args)* }]
            ~~> private_parse_function! {
                $caller
                function = [{ $fn $paren }]
...
macro_rules! private_parse_poly_trait {
    ...
    // Parse angle bracketed lifetimes of poly trait.
    {
        ...
        tt_call! {
            macro = [{ private_parse_lifetime_params }]
            input = [{ $($rest)* }]
            ~~> private_parse_poly_trait! {
                $caller
                poly_trait = [{ $for $lt }]
...
macro_rules! private_validate_inside_brackets {
    // Entry point. Parse content of brackets as type.
    {
        ...
        tt_call! {
            macro = [{ parse_type }]
            input = [{ $($input)* }]
            ~~> private_validate_inside_brackets! {
                $caller
                original = [{ $original }]
            }
        }
    ...
    // Unexpected: bracketed tokens end with semicolon: `[... ; ]`.
    {
        ...
        private_unexpected_close_square_bracket_after_ty_semicolon! {
            $original
        }
    };
...
macro_rules! private_validate_inside_parens {
    ...
    // Entry point. Parse the first element as a type allowing plus.
    {
        ...
        tt_call! {
            macro = [{ private_parse_type_with_plus }]
            input = [{ $($tokens)* }]
            ~~> private_validate_inside_parens! {
                $caller
            }
        }
    };
